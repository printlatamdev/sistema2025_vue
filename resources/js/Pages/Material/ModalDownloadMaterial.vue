            ...
            <form action="" enctype="multipart/form-data">
                <div class="flex" v-if="categories.id == 1">
                    <div class="w-1/2 mr-2">
                        ...
                        <TextInput v-model="store.formDown.code" class="w-full" type="text" />
                    </div>
                        <div class="w-2/4 mr-2">
                            <InputLabel for="company" value="Empresa" />
                            <v-select v-model="store.formDown.company" :options="localCompanies" label="name">
                                <template v-slot:no-options="{ search, searching }">
                                    <template v-if="searching">
                                        No se ha encontrado resultados para <em>{{ search }}</em>.
                                    </template>
                                    <em v-else style="opacity: 0.5">Empieza a escribir para buscar una orden de
                                        compra</em>
                                </template>
                            </v-select>
                        </div>
                ...
                <div v-else>
                    ...
                    <div class=" mt-3">
                        ...
                        <div class="flex bg-gray-50 p-1 rounded-md">
                            <label class="flex items-center">
                                <span v-for="item in orderTypes" :key="item.id" class="mr-5">
                    ...
                    <div class="flex justify-end mt-7">
                        <PrimaryButton @click.prevent="store.downloadMaterial()">
                            <font-awesome-icon :icon="['fas', 'floppy-disk']" class="mr-1" />Descargar {{ categories.id != 1 ? store.isMessage : 'Tinta' }}
                        </PrimaryButton>
                    </div>
            </form>
        </template>
